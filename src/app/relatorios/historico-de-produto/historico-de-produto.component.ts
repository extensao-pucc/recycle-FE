...
export class HistoricoDeProdutoComponent implements OnInit {
  ...
  generateReport(): any{
    ...
    if (formValues.product){
      this.reportManagementService.getDateBeteween('movimentDate', formValues).subscribe(response => {
        this.movimentacoes = response.sort((a, b) => (a.data > b.data) ? -1 : 1);
        this.tempItemsList = _.clone(this.movimentacoes);
        this.showTable = true;
      });
    } else {
      this.toastService.addToast('Selecione o produto para gerar o relatório', 'darkred');
    }
  }
